...
Since LiCOR measurements are often performed multiple times, the code is designed to batch analyze many individual LiCOR runs. Currently, it is assumed that all data to be read adheres to the following format:  
...
- the letter "a" represents an ambient measurement when the tent was not applied,

- the letters "resp" represent a measurement conducted when the tent was applied and shaded,

- the final scenario is the measurement when the tent was applied but unshaded.

It is important to note that net ecosystem productivity can be measured directly from the standard LiCOR output. However, given the experimental protocol of measuring water vapor concentrations during photosynthesis and during respiration, we are, in fact, only measuring evapotranspiration (ET) and evaporation (E), respectively. Thus, we measure transpiration (T) as the difference between evapotranspiration and evaporation, or T = ET - E.
