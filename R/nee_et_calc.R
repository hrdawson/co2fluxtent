...
nee_et_calc <- function(path = NULL, param = "et", skip = 9, vol = 2.197, area = 1.69) {
  ...
  neeet.fit <- function(filename) {
    ...
    # Queery user for finish time for fitting.  Default is set to 80
    # in the if() statement
    ...
    # Calculate nee from linear model.  Factor of 1000 is to convert
    # pressure from kilo-Pascals to Pascals, minus sign is to account
    # for direction of flux.
    if ("nee" == param) {
    ...
    # Use nls2() to scan through parameter grid, searching for "best"
    # actual starting points.  control variable is set to prevent warnings
    # from ending loop.
    ...
    # if/else statement to check for early, peak, late within the parent
    # folder name to indicate season.
    # Now searching for time.  this will be the day/night indicator also
    # in the parent folder name.  just use that.
    ...
    if ("nee" == param) {
      ...
      if (replicate == "y") {
        ...
        return(tibble::tibble(filename, tstart, tfinish, time, camb, tav,
      ...
      if (replicate == "y") {
        ...
        return(tibble::tibble(filename, tstart, tfinish, time, wamb, tav,
